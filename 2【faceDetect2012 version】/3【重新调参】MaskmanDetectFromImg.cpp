...
// 	waitKey();
	...
	thirdCascade.detectMultiScale( smallImg_clone, thirdNestedObjects,
		...
		|CV_HAAR_SCALE_IMAGE
		,
		Size((int)smallImg_clone.cols/12, (int)smallImg_clone.rows/12)
	...
	for( vector<Rect>::const_iterator thirdr = thirdNestedObjects.begin(); thirdr != thirdNestedObjects.end(); thirdr++ )
	{
		...
		if (center.y < smallImg_clone.rows * 0.5 || center.y > smallImg_clone.rows *0.87 || //高度
			center.x < smallImg_clone.cols * 0.4 || center.x > smallImg_clone.cols * 0.7)  //宽度
		{
			//如果鼻子太偏
			//waitKey();
			continue;
...
// 		Size(5,5)
// 		,Size(50,50)
		Size((int)smallImg_clone2.cols/12, (int)smallImg_clone2.rows/12)
	...
	for( vector<Rect>::const_iterator forthr = mouthNestedObjects.begin(); forthr != mouthNestedObjects.end(); forthr++ )
	{
		...
		if (center.y < smallImg_clone2.rows * 0.65
			//|| center.y > smallImg_clone2.rows * 0.9
			)
		{
			//如果嘴巴检测位置太偏
			//waitKey();
