...
void SobelDetect(string imgPath,string imgName)
{
	...
	//threshold(abs_grad_y,abs_grad_y,90,255,THRESH_BINARY_INV);
	//adaptiveThreshold(abs_grad_y,abs_grad_y,255,ADAPTIVE_THRESH_MEAN_C,THRESH_BINARY,25,10);
// 	dilate(abs_grad_y,abs_grad_y,Mat(),Point(-1,-1),1);
// 	erode(abs_grad_y,abs_grad_y,Mat(2,14,CV_8U),Point(-1,-1),1);
...
//	double contArea = contourArea(_contours[0]);//连通域面积
 	Rect r0 = boundingRect(Mat(_contours[0]));
// 	rectangle(afterKeepCon,r0,cvScalar(255,255,255),1,8,0);
