	...
	double scale)
{
	...
	for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++, i++ )
	{
		...
		Point center,center_bak;

		//Scalar color = colors[i%8];
		Scalar color = colors[6];//发现蒙面人以后只显示红色

		int radius,radius_bak;
		...
		//外层是识别出人头 进入
		if(isDetect==true && isDisplayHead==true)
		{
			cout<<"进入为1："<<flag<<endl;
			circle( img, center_bak, radius_bak, color, 3, 8, 0 );
			imshow("kankan",smallImgROI);
			//waitKey(0);
		}
	}
