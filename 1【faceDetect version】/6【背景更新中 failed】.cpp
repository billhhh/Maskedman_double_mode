	...
	if(capture.isOpened()/*capture*/)	//读取文件开关
	{
		...
		for(;;)
		{
			...
			if(!frame.empty())//如果捕捉到了
			{
				...
				accumulateWeighted(gray, learnOut,learningRate,outBinary);
				...
				Mat imageROI;;
				cv::findContours( learnOut, _contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
...
// 					cout<<"C.y == "<<C.y<<endl;
				...
				//画出一个大的框架
				rectangle(learnOut,A,C,cvScalar(255,255,255),1,8,0);
