...
bool isDetect=false;//有没有检测出蒙面人
bool isDisplayHead=false;
...
void detectAndDraw( Mat& img,
	...
	double scale)
{
	int posFaceCnt=0;//看传进来的img里面有木有正脸，没有正脸直接不处理
	headArea hArea[50];
	...
	for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++, i++ )
	{
		...
		Scalar color = colors[6];//发现蒙面人以后只显示红色
		...
		hArea[i].Center=center;
		hArea[i].Radius=radius;
		...
		for( vector<Rect>::const_iterator nr = nestedObjects.begin(); nr != nestedObjects.end(); nr++ )
		{
			...
			//imshow("supSmallImg",supSmallImg);//supSmallImg 正脸图
			cvSkinOtsu(supSmallImg, ssimgResult);
			//imshow("ssimgResult",ssimgResult);

			//判断蒙面人
			findMaskMan(ssimgResult);//ssimgResult 是正脸的二值图

			
		}//识别正脸

		hArea[i].displayHead=isDisplayHead;

	}//识别人头

	cout<<"posFaceCnt == "<<posFaceCnt<<endl;
	...
	for(i=0;i<count;i++)
		if(hArea[i].displayHead==true)
			circle( img, hArea[i].Center, hArea[i].Radius, CV_RGB(255,0,0), 3, 8, 0 );
...
void findMaskMan(Mat img)
{
	...
	if(rate<0.3)
	{
		isDetect=true; //检测到蒙面人
		t = (double)cvGetTickCount();//蒙面人出现时间
		printf( "蒙面人出现时间 = %g ms\n", t/((double)cvGetTickFrequency()*1000.) );
