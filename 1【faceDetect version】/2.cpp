	...
	if(capture.isOpened()/*capture*/)	// 摄像头读取文件开关
	{
		...
		for(;;)
		{
			...
			{ 
				...
				resize(frame,resizeOut,Size( frame.cols/2, frame.rows/2 ),(0,0),(0,0),CV_INTER_AREA);

				//frame->gray 单通道灰度图
				cvtColor(resizeOut, gray, CV_BGR2GRAY);
