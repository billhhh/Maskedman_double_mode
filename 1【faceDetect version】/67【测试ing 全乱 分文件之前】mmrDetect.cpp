	...
	for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++, i++ )
	{
		...
		for( vector<Rect>::const_iterator nr = nestedObjects.begin(); nr != nestedObjects.end(); nr++ )
		{
			...
			if (mouthNestedObjects.size()>0)
			{
				...
				circle( img, center, radius, CV_RGB(128,255,0), 1, 8, 0 );//将嘴巴也画出来，和对应人脸的图形是一样的
...
//测试之用
void detectAndDraw2( Mat& img, CascadeClassifier& cascade,
...
int main()
{
	...
	//Mat img=imread("d:\\faceDetectImg.jpg");
	//detectAndDraw(img,cascade, nestedCascade , thirdCascade ,mouthCascade ,1.0);
	//detectAndDraw2(img,cascade,nestedCascade,2,0);
